...
void ScrollAreaCustom::insertWidget(QWidget *newWidget, bool setAnimation, int n)
{
    newWidget->setParent(container);
    container->InsertWidget(newWidget, setAnimation,n);
}
...
void ScrollListContainer::InsertWidget(QWidget *widget, bool setAnimation,int n)
{
    //Add animation for all widgets current
    QVector<QWidget*> stack;
    for(int i=0;i<n;i++){stack.push_back(widgets.back());widgets.pop_back();}
    AddWidget(widget);
    for(int i=0;i<n;i++){widgets.push_back(stack.back());stack.pop_back();}
}
