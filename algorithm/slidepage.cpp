...
SlidePage::SlidePage(SlidePage *&i):
    QWidget(i->mainparent),
    cornerRadius(i->cornerRadius),
    pageName(i->pageName)
{
    mainparent=i->mainparent;
    //if(parent)
    //    resize(parent->width() * 0.8 <= preferWidth ? parent->width() * 0.8 : preferWidth, parent->height());
    resize(i->mainparent->width() * 0.3 <= preferWidth ? preferWidth : i->mainparent->width() * 0.3, i->mainparent->height());
    this->move(QPoint(-this->width() - 30, 0));
...
void SlidePage::InsertContent(QWidget *widget, int n){widget->setParent(this);pageContentContainer->insertWidget(widget, false,n);}

void SlidePage::AddContent(QWidget *widget){widget->setParent(this);pageContentContainer->addWidget(widget, false);buffer.enqueue(widget);}

void SlidePage::AddContents(QVector<QWidget *> widgets){pageContentContainer->addWidgets(widgets);}

void SlidePage::RemoveContents(QVector<QWidget *> widgets){for(int i = 0; i < widgets.size(); i++)pageContentContainer->removeWidget(widgets[i]);}

void SlidePage::UpdateContents(){pageContentContainer->updateHeight();}

void SlidePage::ScrollToTop(){pageContentContainer->scrollToTop();}
